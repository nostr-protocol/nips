...
1. Measure the address as the UTF-8 byte length of `JSON.stringify(address)` (quotes and JSON escapes included). Escapable characters such as quotes and backslashes inflate this beyond the raw UTF-8 length, so the cap is on the JSON-encoded form. The address MUST be strictly shorter than 204 bytes when JSON-encoded, leaving room for at least one space of padding, and MUST NOT have trailing spaces, since spaces delimit the padding.
2. Pad the address with trailing space (`0x20`) characters so that `JSON.stringify(padded)` is exactly 204 UTF-8 bytes.
3. Wrap the padded address as the compact JSON serialization (no extra whitespace) of:

```json
{"content":"<padded address>","tags":[["platforms","betwithfriends"]]}
```

That is, `JSON.stringify({ content: padded, tags: [["platforms", "betwithfriends"]] })`. The wrap adds 52 bytes of framing around the 204-byte JSON-encoded `content` value, so the [NIP-44](44.md) plaintext is always 256 bytes and fills one padding bucket; ciphertext length therefore reveals nothing about the address. The `platforms` tag identifies the payload so cooperating signers can restrict decryption to this protocol.

4. Encrypt that JSON string with NIP-44 v2 from the bettor's key to the admin's pubkey.
5. Decode the resulting base64 string to raw bytes and encrypt them under the pool key into the `nipx-address` tag as described above.

To read an address, the admin decrypts the `nipx-address` tag with the pool key, computes the NIP-44 conversation key from their own key and the bettor's pubkey (the embedded zap request's `pubkey`), decrypts, parses the JSON, reads `content`, and strips trailing spaces. Admins SHOULD check that the result is a plausible lightning address before paying it, since nothing forces a bettor's client to encrypt a meaningful one.

With the fixed 204-byte JSON-encoded padding plus the constant wrap, every `nipx-address` tag value is a constant length regardless of the actual address.
