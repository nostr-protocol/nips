...
`i` tags are used for referencing these external content ids, with `k` tags representing the external content id kinds so that clients can query all the events for a specific kind. Multiple `k` tags may be used for additional filtering.
...
Additional podcast `k` tags MAY be added to reference the [medium](https://github.com/Podcastindex-org/podcast-namespace/blob/main/docs/tags/medium.md) of the RSS feeds, and allow querying notes related to specific RSS mediums such as music. These medium tags are optional and if they are not present the medium is assumed to be `podcast:medium:podcast`. If missing clients SHOULD resolve the RSS feeds and check the medium tag to be sure.

- `["k", "podcast:medium:music"]` - for music feeds
- `["k", "podcast:medium:podcast"]` - for podcast feeds
