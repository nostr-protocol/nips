...
Allow 'all of nostr' to reference, comment, and react on any published NIP spec in a standardized manner, regardless of which repo or nostr event kind they are posted to. The `i` tag value SHOULD be in a standard format matching `a` tags, as specified in NIP-01. If referencing a published Nostr event that contains a NIP proposal, the format SHOULD be`<kind integer>:<32-bytes lowercase hex of a pubkey>:<d tag value>`. If referencing a NIP proposal file in an online repository, the format SHOULD be `<repository domain>:<repository ownername>:<local NIP identifier>`. The `i` tag value MAY be a single finalized NIP number, but this use is deprecated. A URL hint MAY be added at end of the `i` tag, to suggest a web page without hard linking the reference to a specific repo or nostr client. 
...
// <repository domain>:<repository ownername>:<local NIP identifier>
[
  ["i", "github.com:nostr-protocol:25", "https://github.com/nostr-protocol/nips/blob/master/25.md"],
  ["k", "nip"]
]
