...
| Blockchain Name        | "`<blockchain>`:name:`<namespace>`/`<name>`"               | "`<blockchain>`:name"    |
...
`<blockchain>` can be any layer 1 chain (`bitcoin`, `ethereum`, `solana`, `namecoin`, ...). If necessary (e.g. for ethereum), you can specify a `<chainId>`. Naming chains (e.g. `namecoin`) additionally expose a `name:` artifact type alongside `tx:` and `address:`.
...
```
namecoin:name:<namespace>/<name>
```

Namecoin (and other naming chains) expose names as a primary on-chain artifact alongside transactions and addresses. The `<namespace>` is the Namecoin namespace prefix (e.g. `d` for Domain Name Object names, `id` for identity names — see [ifa-0001](https://github.com/namecoin/proposals/blob/master/ifa-0001.md) for both), and `<name>` is the name string without the namespace prefix (i.e. the part that follows `<namespace>/` in the on-chain key).

E.g. the `d/example` name (resolving as `example.bit`):
...
And the `id/alice` identity name:

```jsonc
[
  ["i", "namecoin:name:id/alice"],
  ["k", "namecoin:name"]
]
```

The `<namespace>` and `<name>` are case-sensitive and MUST match the on-chain key byte-for-byte. Namecoin name lookups are typically performed via [ElectrumX](https://github.com/namecoin/electrum-nmc) or an indexer; see the linked Namecoin proposals for the canonical resolution semantics.
