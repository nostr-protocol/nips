...
After decryption, the inner rumor MUST be a structurally valid unsigned event
containing exactly `id`, `pubkey`, `created_at`, `kind`, `tags`, and `content`.
It MUST NOT contain `sig` or any other top-level field, and its `id` MUST equal
its recomputed [NIP-01](01.md) event id.
...
handleNostrEvent(e: NostrEvent) {
  ...
  const innerEvent = parseExactUnsignedRumor(plaintext);
  if (
...
Serialized session state contains private ratchet keys, chain keys, skipped
message keys, and the root key. Applications MUST protect its confidentiality
and integrity at rest, and MUST NOT deserialize session state received from an
unauthenticated source. They MUST prevent rollback to older snapshots and
concurrent use of multiple copies of the same session state.
