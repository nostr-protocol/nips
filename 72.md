...
While sending a non kind `1` event, the clients MUST populate the `content` field of the event with the _the stringified JSON of the event_.  This is similar to the ones used for reposts in [NIP-18](18.md).

Additionally, for non kind `1` events, the clients MUST add a `k` tag with value of the kind of event to be posted.
