...
Users can declare their control over one or more DNS-based internet identifiers in kind `10008`. 
...
The property `names` MUST contain a map from names to lowercase hex-formatted public keys. 

The property `relays` is optional, but when present, it MUST contain a map from pubkeys to an array of relay URLs where events for that key can be found. 

A valid NIP-05 identifier MUST match the resulting public key in the JSON to the pubkey of the `10008` event.
...
Servers MUST provide their `/.well-known/nostr.json` documents with the HTTP header `Access-Control-Allow-Origin: *` to ensure they can be validated by pure JS apps running in modern browsers, avoiding [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) issues.
...
For example, if after finding that `bob@bob.com` has the public key `abc...def`, the user clicks a button to follow that profile, the client must keep a primary reference to `abc...def`, not `bob@bob.com`. If, for any reason, the address `https://bob.com/.well-known/nostr.json?name=bob` starts returning the public key `1d2...e3f` at any time in the future, the client must not replace `abc...def` in his list of followed profiles for the user (but it should stop displaying "bob@bob.com" for that user, as that will have become an invalid NIP-05 address).
...
### Reasoning for the `/.well-known/nostr.json?name=<name>` format

By adding the `<name>` as a query string instead of as part of the path, the protocol can support both dynamic servers that can generate JSON on demand and static servers with a JSON file in it that may contain multiple names.
