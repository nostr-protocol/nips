...
If a TXT record for `_ignoreme._nostr.<domain>` exists the fetcher MUST NOT send NIP-05 requests from this domain.

The `/.well-known/nostr.json` endpoint MUST NOT redirect cross origin. 

Fetchers MUST ignore any cross origin redirects on the `/.well-known/nostr.json` endpoint.
