...
### Optional Multi-Identifier Support

Clients MAY query and validate multiple NIP-05 identifiers per pubkey, where supported by extending kind `0` events with an optional `"nip05_aliases"` key containing an array of internet identifiers.
...
```jsonc
{
  ...
  "nip05_aliases": ["gamerbob@service.net", "maxibob@nostrmaxi.com"]
  // other fields...
}
```

Upon seeing `"nip05_aliases"`, the client SHOULD split and query each identifier as described above (e.g., `https://service.net/.well-known/nostr.json?name=gamerbob`). If the pubkey matches for an alias, the client MAY display it alongside the primary identifier (e.g., in a profile list with validation status) and use it for resolution in searches.

Clients SHOULD limit processing to 100 aliases as a guideline to prevent abuse. NIP-05 identifiers remain resolution pointers only; basic multi-domain mappings require no profile changes, as pubkeys can already be associated via domain-side `nostr.json` configurations.
