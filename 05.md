...
The NIP-05 identifier with single-char `~` as `<local-part>` value is considered a HTTP-only ID that
doesn't need a corresponding nostr pubkey linked to a kind-0 event to be valid.

An HTTP service identified by it should add a `profile` key to the `https://<domain>/.well-known/nostr.json`
JSON response containing any of the kind-0 fields. For example:

```json
{
  "profile": {
    "lud16": "an@example.com",
    "name": "Example File Storage Service",
    "picture": "https://example.com/images/logo.jpg"
  },
  "names": {
    "bob": "b0635d6a9851d3aed0cd6c495b282167acf761729078d975fc341b22650b07b9"
  },
