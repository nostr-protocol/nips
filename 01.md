...
- The format for sending events is not perfect:
  - `id` is redundant. The relay/client has to compute it in the process of
    verifying the signature anyway
  - `"content":""` is redundant and so is `"tags":[]`
  - `""` around the JSON keys is redundant
  - an array of 7 or 8 values could save us sending the JSON keys
  - [relaxed json](http://www.relaxedjson.org/) even does away with `""` around
    simple values and `,` between arrays and objects
  - [BSON](https://bsonspec.org/) would store event ids and pubkeys at least 50%
    more efficiently
  - If we invent our own binary format, we could probably save 60% on most
    events
  - ... each of these "improvements" would complicate the development of new
    tools, even if optional, gaining us very little where it really matters. All
    of those things were proposed at some point and so far not standardized.
    Think twice before working out a nip!
