...
Relays can send 6 types of messages, which must also be JSON arrays, according to the following patterns:

  * `["HELLO", { "self": "<relay pubkey>", "feature_x": { ...feature_x_specific_fields }, "feature_y:" { ...feature_y_specific_fields } }]`,
  used to let the client know what extra features besides those on NIP-01 the relay supports.
  It's an empty JSON if there are no such features.
  It MUST be the first relay message sent upon connection because the client can choose to wait for it before sending their own messages or deciding to disconnect.
