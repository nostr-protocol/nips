...
- The `a` tag, used to refer to a replaceable event.
    ...
    - Structure of a parameterized event: `["a", <kind integer>:<32-bytes lowercase hex of a pubkey>:<d tag value>, <recommended relay URL, optional>]`
    - Structure of a non-parameterized event: `["a", <kind integer>:<32-bytes lowercase hex of a pubkey>:, <recommended relay URL, optional>]`
