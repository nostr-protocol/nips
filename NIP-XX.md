...
2. Validate the request structure per §Request Fields.
3. Resolve the recipient's `sp1...` address from `recipient_npub` using the
...
4. Select UTXOs from the wallet's available balance sufficient to cover
   `amount_sats` plus estimated fees at the requested `fee_rate`.
5. Construct the BIP-352 transaction per §Transaction Construction.
6. Sign and broadcast the transaction.
7. Respond with a kind `23353` event per §Response.
8. If `notification_relay` is present, deliver a kind `8352` receipt
...
- **[BIP-375](https://github.com/bitcoin/bips/blob/master/bip-0375.mediawiki)** — PSBT fields enabling hardware wallet signers (BitBox02, Coldcard, SeedSigner) to contribute ECDH shares during Silent Payment output construction. Wallet services SHOULD produce BIP-375-compatible PSBTs when constructing transactions.
