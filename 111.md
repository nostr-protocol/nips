...
Relays may provide addition metadata to clients to inform them of
...
and do not constitute a default value or expectation. The purpose
...
- `max_raw_json`: this is the maximum number of bytes for incoming JSON that the relay
...
- `default` is normal retention time for any event, if not overriden by other fields.
...
- `only_swf` is an boolean that only "Safe For Work" content is encouraged on this
  relay. This relies on assumptions of what the "work" "community" feels 
  "safe" talking about.
...
The existing NIP-11 field `description`, should be used to describe
your community goals and values, in brief. The `posting_policy` is
for more additional detail.
