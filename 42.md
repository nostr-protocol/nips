...
- `["AUTH", <challenge-string>]`: Just two array elements. It means relay requires authentication for any resource access.
...
- `["AUTH", <challenge-string>, "REQ", <subscription_id>]`: It references a previous client `REQ` message with a specific subscription id.
It means client subscription potentially requested atleast one event that requires prior authentication to be accessed.
...
- `["AUTH", <challenge-string>, "EVENT", <event_id>]`: It references a previous client `EVENT` message with a specific event id as fourth array element.
It means relay requires authentication before publishing any or atleast this kind of event.
MUST be sent before a failure `OK` message with same event id.
