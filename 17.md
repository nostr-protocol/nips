...
Clients SHOULD randomize the `created_at` as `random(now - latest_created_at) + latest_created_at` with `latest_created_at = max(last_message_created_at, latest_10017_created_at)` to avoid publicly exposing the exact timing of messages. Random updates to this event, simulating fake receipts, are also recommended.

Clients SHOULD listen to changes in the `10017` kind and mark all kinds `14` and `15` messages with their `created_at` smaller than the `10017`'s `created_at` as received.
