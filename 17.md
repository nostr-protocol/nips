...
A public (not giftwrapped) event of kind `10017` SHOULD be updated every time a client decrypts a new message. This update will mark all messages until this point as "received" and confirm to the user's chat groups that the user is still using a NIP-17 client, has received and can see their messages.
...
Clients SHOULD listen to changes in the `10017` kind and mark all kind `14` and `15` messages with their `created_at` smaller than the `10017`'s `created_at` as received.
