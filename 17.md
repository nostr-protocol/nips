...
`kind:14` is defined as a chat message. It is designed to be a Rumor in [NIP-59](59.md) and thus MUST never be signed. If it is signed, the message becomes **fully public**. This event kind requires [NIP-59](59.md) encapsulation to make it private.  
...
The content MUST follow the same plaintext rules of `kind:1`. Tags that mention, quote and assemble threading structures MUST follow [NIP-10](10.md). Fields `id` and `created_at` are required. 
...
Clients SHOULD randomize `created_at` in up to two days in the past in both the seal and the wrap to make sure grouping by `created_at` doesn't reveal the group's metadata. 
...
Kind `10050` indicates the user's preferred relays to receive DMs. The event MUST include a list of `r` tags with relay URIs.
...
Clients SHOULD guide users to keep `kind:10050` lists small (2-4 relays) and SHOULD spread it to as many relays as viable. 
