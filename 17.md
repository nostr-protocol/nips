...
Kind `14` is a chat message. `p` tags identify the receivers of the message.
...
`.content` MUST be plain text and tags that mention, quote and assemble threading structures MUST follow [NIP-10](10.md). Fields `id` and `created_at` are required. 
