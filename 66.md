...
- `W` - A relay attribute sourced from the relay's [NIP-11](11.md) `attributes` field. Monitors SHOULD populate one `W` tag per entry in the relay's `attributes` array, preserving the original value. This enables attribute-based relay discovery by allowing clients to filter `30166` events by `#W`.
