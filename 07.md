...
async window.nostr.nip44.getConversationPublicKeys([pubkey1, pubkey2, ...]): string[] // takes array of pubkeys, returns array of conversation public keys
async window.nostr.nip44.signEventWithConversationKey(pubkey, event: { created_at: number, kind: number, tags: string[][], content: string }): Event // takes a pubkey and event object, adds `id`, `pubkey` and `sig` using the conversation key as the private key and returns it
```
