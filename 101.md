...
|---------|------|-----------|
| 30504 | WoTx2 Training Resource | [42-oracle-permits-extension.md](42-oracle-permits-extension.md) |
| 30506 | Mediation Dossier (WoTx²) | [56-friction-mediation-extension.md](56-friction-mediation-extension.md) |
| 30508 | ATOM4LOVE Vibratory Match | [101-atom4love-extension.md](101-atom4love-extension.md) |
| 30850 | Economic Health Report | [101-economic-health-extension.md](101-economic-health-extension.md) |
| 30851 | ZEN Emission Proof (OC payment) | [101-economic-health-extension.md](101-economic-health-extension.md) |
...
## UENC — Encrypted File Format for IPFS

UPlanet defines a binary format **UENC v1** for files stored encrypted on IPFS. This enables private media sharing over decentralized storage while keeping the encryption key out of the public record.

### Binary Layout

```
+----------+--------+---------+----+------------------+
| MAGIC(4) | VER(1) | TYPE(1) | IV | CIPHERTEXT + TAG |
+----------+--------+---------+----+------------------+
  "UENC"     0x01    0x01(AES) 12B   N + 16 bytes
```

| Field | Size | Value |
|-------|------|-------|
| MAGIC | 4 | `0x55 0x45 0x4E 0x43` ("UENC") |
| VERSION | 1 | `0x01` |
| ENC_TYPE | 1 | `0x01` = AES-256-GCM |
| IV | 12 | Random initialization vector (GCM nonce) |
| CIPHERTEXT+TAG | N+16 | AES-256-GCM output (16-byte tag appended) |

The IV is embedded in the file so the recipient needs only the 32-byte AES key (transmitted out-of-band via NIP-44 DM).

### Upload API

```http
POST /api/fileupload/encrypted
Content-Type: multipart/form-data

file=<binary>
encryption_key=<64-hex-chars>   ; 32 bytes, generated client-side
...
{
  "success": true,
  "cid": "Qm...",           // IPFS content identifier of the UENC blob
  "encryption_type": "aes256gcm",
  "iv_hex": "...",          // 24-char hex (informational, also embedded in file)
  "original_filename": "photo.jpg",
  "original_size": 123456,
  "encrypted_size": 123490
}
```

Headers: `X-Encryption-Type`, `X-Encrypted-CID`, `X-Encryption-IV`.

The server **never persists the key** — it encrypts in memory and discards the key after the response.

### Key Transport via NIP-44

The AES key is transmitted inside a NIP-44 encrypted DM. Two use-cases:

**1. LOVE channel (user ↔ BRO station)**
...
```jsonc
{
  ...
  "payload": {
    "action": "image",
    "text": "{\"cid\":\"Qm...\",\"enc_key\":\"<64-hex>\",\"enc_type\":\"aes256gcm\",\"filename\":\"photo.jpg\",\"title\":\"Sunset\",\"caption\":\"Beautiful 🌅\"}"
  }
}
```

**2. Peer-to-peer DM (NIP-44 kind:4)**
...
```jsonc
{
  "_uenc_img": {
    ...
    "caption": "Optional caption with emoji 🎉"
  }
}
```
...
- **IPFS file**: encrypted blob — CID alone reveals nothing without the key
- **AES key**: lives only inside the NIP-44 DM (E2E encrypted between sender and recipient)
- **Server**: stateless — key is never stored or logged
- **Receiver**: decrypts in-memory, displays image; key never persists to disk

### Image Composition

When a user attaches an image in Ẑelkova (wallet), a composition sheet collects:
- **Title** (optional) — short label shown above the thumbnail
- **Caption** (optional) — multi-line text with full emoji picker (all Unicode blocks, not limited like some platforms)

Both fields accept emoji via the integrated picker. The title and caption travel inside the NIP-44 payload alongside the CID and key.

### Client-Side Reference (Ẑelkova / Dart)

```dart
// Generate key
final key = EncryptedFileService.generateKey(); // 32 random bytes
// Upload
final result = await EncryptedFileService.upload(
  fileBytes: imageBytes,
  filename: 'photo.jpg',
);
// result.encKeyHex + result.cid  →  send in NIP-44 DM

// Receive & decrypt
final plaintext = await EncryptedFileService.downloadAndDecrypt(
  cid: result.cid,
...
The NIP-101 strfry write-policy plugin does **not** inspect the content of NIP-44 DMs (kind 4) — UENC image messages pass through without special handling. The IPFS CID is opaque to the relay.

## Compatibility

- ✅ Uses standard event kinds where possible (0, 1, 3, etc.)
- ✅ UENC encrypted files use standard IPFS content-addressing (CID)
...
- [Ẑelkova](https://github.com/papiche/zelkova) (AGPL-3.0) — Flutter wallet with UENC image messaging
