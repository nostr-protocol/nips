...
```yaml
{
    ...
    "params": {
        ...
        "max_fee": 456, // maximum routing fee the sender is willing to pay, in msats, optional
...
The `max_fee` parameter caps the total routing fees a **wallet service** may pay on top of the invoice `amount`. If a route cannot be found whose total cost does not exceed `amount + max_fee`, the wallet service MUST NOT send the payment and MUST return a `FEE_LIMIT_EXCEEDED` error.

A wallet that honors `max_fee` MUST include the actual `fees_paid` in the response (already defined), so the client can reconcile against the budget. Wallets that do not implement fee limits MUST ignore the parameter, in which case the client should treat the absence of `fees_paid` in the response as a signal that the `max_fee` budget may not have been enforced.

The `max_fee` budget SHOULD encompass the full Lightning routing fee delta between the amount debited from the payer's balance and the invoice amount; it MUST NOT include the invoice amount itself.
...
- `FEE_LIMIT_EXCEEDED`: No route was found within the `max_fee` budget. The wallet MUST NOT have debited funds.
