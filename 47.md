...
>_I've put some comments below_+
...
>_The term `Nostr app` is confusing.  Do we talk to it over a websocket using the nostr protocol?  Or it is a web app that responds to http requests?_
...
>_This is very confusing.  The implication of the first paragraph is that the client contacts the wallet service over http.  The second paragraph says that the wallet service generates the connection URI.  Does this mean that the connection URI is the response to the http request?_
...
>_Again, this implies an http request.  I presume this is just a repeat of the URI section above and that this is done once and then stored.  Perhaps that's what you meant by numbering this item 0?_
...
>_Now I'm lost.  How do I send a nostr event to the wallet service?  Is the wallet service now a websocket?  How did I get that websocket?  If the wallet service is not a websocket then what it is and how do I format a nostr event to send to it? _
