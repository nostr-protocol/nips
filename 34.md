...
An attempt SHOULD be made to push tip to `refs/nostr/<[PR|PR-Update]-event-id>` for each [grasp server](https://njump.me/naddr1qvzqqqrhnypzpgqgmmc409hm4xsdd74sf68a2uyf9pwel4g9mfdg8l5244t6x4jdqy28wumn8ghj7un9d3shjtnwva5hgtnyv4mqqpt8wfshxuqlnvh8x) identified in the repository's announcement event's `"clone"` tag using identification method: `clone` tag includes `[http|https]://<grasp-path>/<valid-npub>/<string>.git` and `relays` tag includes `[ws/wss]://<grasp-path>`.
...
```jsonc
{
  ...
  "tags": [
    ...
    ["clone", "<clone-url>", ...] // optional git clone urls where commit id can be downloaded if not available from repoistory announcement clone urls
  ]
}
```
