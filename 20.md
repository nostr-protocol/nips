...
The `message` SHOULD start with `pow-adaptive:` if the event doesn't meet proof-of-work difficulty for that particular pubkey, IP or event. The client MAY mine the new event and MAY resubmit the event.
