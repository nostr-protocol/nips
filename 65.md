...
The event MUST include a list of `r` tags with relay URLs as value and an optional `read` or `write` marker. Relays marked as `read` are to be considered **read** relays, relays marked `write` are **write** relays, if the marker is omitted the relay is both **read** and **write**.
...
When downloading events **about** a user, where the user was tagged (mentioned), clients SHOULD use their **read** relays.
