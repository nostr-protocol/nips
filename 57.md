...
6. If the `zap request` is valid, the server should fetch an invoice whose description hash is the SHA256 hash of the JSON-encoded `zap request`. The server MUST remember the JSON-encoded `zap request` as the description preimage for the invoice. No additional lnurl metadata is included in that description preimage. This invoice will be returned in the response according to [LUD06](https://github.com/lnurl/luds/blob/luds/06.md).
...
A `zap receipt` is created by a lightning node when an invoice generated by a `zap request` is paid. `Zap receipt`s are only created when the invoice description hash commits to a `zap request` note.

When receiving a payment, the following steps are executed:

1. Get the saved description preimage for the invoice. This needs to be saved somewhere during the generation of the description hash invoice. It is saved automatically for you with CLN, which is the reference implementation used here.
2. Parse the saved description preimage as a JSON nostr event. This SHOULD be validated based on the requirements in Appendix D, either when it is received, or before the invoice is paid.
