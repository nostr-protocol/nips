...
`9734` is a _zap request_, representing a payer's request to a recipient's lightning wallet for an invoice.
`9735` is a _zap receipt_, representing the confirmation by the recipient's lightning wallet that the invoice issued in response to a zap request has been paid.
...
4. Instead of publishing the _zap request_, the `9734` event should be sent to the `callback` URL received from the `lnurl` pay endpoint for the recipient using a `GET` request.
...
This event's `content` **MAY** be an optional message to send along with the payment. The event **MUST** include the following tags:
...
- **`amount`:** the amount in _millisats_ the sender intends to pay
- **`nostr`:** the `9734` zap request event, JSON encoded then URI-encoded
- **`lnurl`:** the `lnurl` pay url of the recipient, encoded using `bech32` with the prefix `lnurl`
