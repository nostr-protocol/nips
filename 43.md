...
Relays MUST ensure that membership state events for a given pubkey are mutually exclusive.

For a given member pubkey, only one of the following events MAY exist at any time:

- `kind 8000` — indicating the user is a member
- `kind 8001` — indicating the user has been removed

When publishing a new membership state event, the relay MUST remove the opposing event type for that pubkey.

Specifically:

- When a relay publishes a `kind 8000` event for a pubkey, any existing `kind 8001` event for that pubkey MUST be removed.
- When a relay publishes a `kind 8001` event for a pubkey, any existing `kind 8000` event for that pubkey MUST be removed.

This ensures that a relay maintains a single authoritative membership state per user.
