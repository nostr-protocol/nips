...
A user can create a `kind 360` `precommit` event which lists one or more public keys that are authorized in advanced as possible `migration` keys. Designated private keys MUST be stored securely.
...
In order to migrate a key, the original user MUST contact each shard holder out of band to recover the key material and use it to sign and publish a `kind 361` `migration` event.
