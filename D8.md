...
This event MAY include a message in the `content` field. Clients SHOULD fetch this event and notify its users that the key has been invalidated.
...
2. Fetch all `kind 260` events signed by the `target` key to identify the `target`'s `ratchet` key.
...
Because a single `ratchet` key can create and invalidate many `authorized` keys, this NIP may be used to generate single-use "device keys", which can be invalidated if compromised. However, this should be used sparingly, since proliferation of keys introduces a denial of service vector for implementations attempting to validate keys.

### Denial of Service Risk

This NIP imposes a soft cap of 16 `authorized` keys per user. Implementations SHOULD stop validating keys after this point, and users SHOULD avoid creating more than 16 different keys. This limit is intended to leave some room for mistakes and multiple rotatations, but prevents users from spamming key rotations.
...
Clients SHOULD implement key validation. Relays MAY implement key validation and MAY drop invalid events from their database.
...
Alice then invalidates key `A` by publishing a `kind 262` event signed by key `A'`, including an `as_of` tag set to the moment the attacker first signed a message with her key, as well as a message to followers to migrate to key `B`:
...
She also requests a `kind 1040` OTS attestation which `e`-tags event `A'.2` to be published to her inbox relays.
...
```typescript
[{"kinds": [1], "authors": ["<A>"], until: "<A'.2.as_of>"},
 {"kinds": [1], "#=": ["<A>"]}]
```
...
- `A` remains her `identity` key, but is no longer `authorized` as of `A'.2.as_of`.
- `A'` (her first `ratchet` key) is invalid as of `A'.2.as_of`.
