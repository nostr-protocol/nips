...
| Enrichment Confidence     | `confidence`           | float, norm 0-1       |
...
The `confidence` tag is the provider's overall confidence in the enrichment. It MUST represent a finite number from `0` through `1`. NIP-01 requires tag elements to be strings, so clients parse the value as a number:
