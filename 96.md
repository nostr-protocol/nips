...
```js
{
  "nip96": {
    // required
    "routes": {
      // If the protocol is https it can be omitted from the value
      "upload": "upload.domain/upload-path",
      "dowload": "download.domain/download-path",
      "deletion": "http://deletion.domain/deletion-path"
    }
    // optional
...
The `server` MUST reject with 401 Unauthorized status if some fiels are invalid.
...
```js
{
  nip96: {
    ...
    dowload_route: "download.domain/download-path",
...
[
  ...
  // If the protocol is https it can be omitted from the value
  ["nip96", "server.example.domain"]
]
```

The `nip96` tag is an important hint that clients can use to find new NIP-96 compatible file storage servers.

If the file upload was made to more than one `server`, `r` query params must be added to the url string, containing each of the `nip96.dowload_route` response values from `servers` used in **successful** upload operations'. The `nip96` tag should also include all the server hostnames that hosts the /.well-known/nostr.json resource. For example:

```js
[
  ["url", "https://download.domain/download-path/<sha256-file-hash>.png?r=download.domain2%2Fdownload-path2&r=download.domain3%2Fdownload-path3"],
  ["x", "<the-hash>"],
  ["nip96", "server.example.domain", "server.domain2", "server.domain3"]
]
```

Alternativelly, instead of using NIP-94, the `client` can share or embed on a nostr note just the above url with added "x" and "nip96" [NIP-54](54.md) inline metadata fields and optionally other fields such as "magnet".
...
`Clients` should send a `GET` request to the server url in the format `https://download.domain/download-path/<sha256-file-hash>.png?r=download.domain2%2Fdownload-path2&r=download.domain3%2Fdownload-path3`.

If the `server` doesn't have the file anymore, it MUST reply with a `302` HTTP redirect to the next `server` domain included as `r` query param. For example, issue a redirect to `https://download.domain2/download-path/<sha256-file-hash>.png?r=download.domain3%2Fdownload-path3`. The url used for the redirect SHOULD be limited to 3 `r` query params, so to avoid excessive number of redirects.
...
Example: `https://download.domain/download-path/<sha256-file-hash>.png?r=download.domain2%2Fdownload-path2&w=32`
...
The response is similar to the json sent on uploads, but without the `nip96.dowload_route` field.
...
`Clients` who make `zaps` available to their users are expected to try accessing the `/.well-known/nostr.json?name=_` path of all downloaded media domains from the NIP-94 `nip96` tag or NIP-54 fragment field and, if available, get the zap related metadata (such as lud06 address).
...
it must respond with a 422 Unprocessable Entity status.
...
It also must add `url`, `x`, `nip96` and `nip60` tags (the latter signals
