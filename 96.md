...
Note that if the server didn't apply any transformation to the received file, both `nip96.ox` and `nip96.x` fields will have the same value. The server MUST link the saved file to the SHA-256 hash of the **original** file before any server transformations (the `nip96.ox` tag value). The **original** file's SHA-256 hash will be used to identify the saved file when downloading or deleting it.
...
The primary file download url informed at the upload's response field `nip96.nip94_event.tags.*.url`
