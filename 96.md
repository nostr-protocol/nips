...
`draft` `optional` `author:arthurfranca` `author:Semisol` `author:staab` `author:fishcakeday`
...
```js
{
  ...
  // Optional
  "accepted_mime_types": ["image/jpeg", "video/webm"]
  // Optional
  "plans": {
...
```js
{
  status: "success", // or "error" if unsuccessful
  nip96: {
    code: 200, // or other HTTP status code
    messages: {
      // Can be an empty array if successful
    ...
    // users will try to download the transformed file using this value
    ...
    // The server can but doesn't need to store it.
...
Alternatively, instead of using NIP-94, the `client` can share or embed on a nostr note just the above url with added "x" [NIP-54](54.md) inline metadata field values and optionally other fields such as "xx" and "m" for mime type.
