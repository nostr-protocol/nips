...
```js
{
  ...
  // or "processing" for special case described at "Delayed Processing" section
  status: "success",
...
Sometimes a server may place the uploaded file in a processing queue. In this case the server should reply with **202 Created** and the following JSON:

```
{
  status: "processing",
  message: "Processing. Please check the processing url for updated status.",
  processing_url: "..."
}
```

The `processing_url` field is an url the client can use to check if the processing is done.
...
When the processing is over, the server replies at the `processing_url` url with a regular successful 200 OK status JSON response already mentioned before.

**Alternatively**, for a simpler flow, the server should serve the original file while the processing isn't done, then swap the original file for the processed one when the processing is over. The upload response is the same as usual but some optional metadata like `nip94_event.tags.*.x` and `nip94_event.tags.*.size` won't be available.
