...
```js
{
  "nip96": {
    ...
    // optional
    "terms_of_service": "...",
    // optional / range in days / -1 for no expiration
    // [7, -1] means it may vary from min 7 days to unlimited persistence
    // [-1, -1] means it has no expiration
...
For example, the file storage `server` may fetch [zap receipts](57.md#appendix-e-zap-receipt-event) from its
zap provider pubkey and extract embedded media from referenced notes. Stored files that haven't received zaps
...
It's advisable to cache payer pubkeys for faster authorization the next time the user attempts to download the file.
