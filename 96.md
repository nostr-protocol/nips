...
`Clients` who make `zaps` available to their users are expected to try to access the `/.well-known/nostr.json?name=_` path of all downloaded media domains (don't forget the domains on `r` query params) and, if available, get the zap related metadata (such as lud06 address).

Then they should cache this link between domain and zap addresses (or lack of). The individual cache entries should be renewed if too old.

A nostr note with one or more embedded files from zap-enabled domain(s) should be considered zappable.

If more than one wallet address is detected for a nostr note, zap splits should take place if Nostr Wallet Connect is configured by the user (as per [NIP-47](47.md)).

If NIP-47 isn't configured, one of the addresses should be randomly chosen to receive the zap,
which is statistically fair (it is much better than expecting user to pay many invoices at once because it would be bad UX).

Nostr note author wallet address can be used too when splitting zaps.
