...
A relay that implements this NIP MAY append a third element to the `EOSE` message:
...
When a relay that advertises support for this NIP omits the hint, the client SHOULD treat the response as not complete and paginate using `until = oldest received event's created_at` as usual.
...
Future revisions of this NIP may define additional hint values or allow the third element to be an object for richer pagination metadata. Implementations SHOULD accept unknown hint values without error and treat them as if no hint were present.
