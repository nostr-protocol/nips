...
## Deletion of Gift Wrap Events by Recipient

Gift wrap events (`kind:1059`, see [NIP-59](59.md)) use an ephemeral `pubkey`, not the recipient's. This makes it impossible for recipients to request deletion of gift wraps addressed to them under the standard same-`pubkey` rule.

A `kind:5` deletion request MAY reference a `kind:1059` event when the deletion request `pubkey` appears in a `p` tag of the referenced gift wrap, even if it does not match the gift wrap's `pubkey`. The `k` tag MUST be `"1059"`.
...
```jsonc
{
  ...
  "content": "removing my copy",
  // other fields...
}
```

This signals that the recipient no longer wants their copy stored. It does not affect gift wraps addressed to other participants.

Clients MUST verify that the deletion request `pubkey` appears in a `p` tag of the referenced gift wrap before treating it as deleted. Clients MUST NOT honor the request if this check fails.

Relays SHOULD delete or stop serving a `kind:1059` event upon receiving a valid deletion request from a `p`-tag recipient. Relays MAY enforce this check before acting, but are not required to since they may not have all referenced events.
