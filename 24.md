...
7. **No Shared Secrets**: There is no chatroom secret key that can leak or be mistakenly shared
...
11. **Composable**: The protocol is extensible to make any other event kind fully private (private likes, private reports, private long-form content, etc)
...
Tags MUST always be empty. The inner event MUST always be unsigned. The encryption algorithm MUST use [NIP-44](44.md). 
...
Clients pull all GiftWraps citing their user and decrypt the gift-wrapped event with the user's private key to get the Seal and to know where this Gift is coming from. After breaking the seal, the Client can see the message, date/time as well as any pertinent tags. 
...
To protect recipient metadata, relays SHOULD only serve kind 1059 events to the requesting user. If a relay implements some form of authentication it can refuse to serve wrapped events to non-recipients. 
