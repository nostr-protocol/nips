...
`kind:14` is a Chat Message between a group of users (i.e. pubkey + p-tags). It is designed to be a Rumor in [NIP-59](https://github.com/nostr-protocol/nips/pull/716/files) and thus MUST never be signed. If it is signed, the information in it is **fully public**. This event requires the rest of the protocol to make it private. 
...
The `.content` contains the message. The set of `pubkey` + `p` tags define a group. Clients SHOULD render messages with the same group in a continuous thread. If a new p tag is added, a new group is created. 
...
The use of other tags is encouraged, such as `content-sensitivity` if the content is NSFW. 
...
A GiftWrap event (`kind:1059`) wraps all `kind:13` events. It is signed by a single-use (i.e. random) key and it is `p`-addressed to the receiver. 
