...
9. The protocol's messages must flow through public relays. Private relays can increase privacy further, but they are not needed.
...
A Sealed Gossip is a `kind:13` event that wraps an unsigned event (the Gossip) with the sender's regular key. The Sealed Gossip is **always** encrypted to a receiver's pubkey but there are no `p-Tag` associated with this event. There is no way to know who the Gossip is for without the receiver or the sender's private key. The only public information in this event is who is signing it. 
...
Private Group chats are `Kind:14`s for more than one `p` tag. There is no channel definition event. The group of authors + p' tags is a group. 
