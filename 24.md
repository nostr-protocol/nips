...
3. Sender and Receivers cannot be linked with public information alone, not even their random keys.
4. There is no central queue, channel or otherwise converging event id to link all messages from the group. 
5. Minimal trust in counter parties: They cannot expose the details of your message, including the metadata, without exposing their entire profile (private key)
...
8. Users and clients can opt-in for Dissapearing Messages that are not recoverable with the private key
9. The protocol's messages must flow through public relays. Private relays can increate the privacy further, but they are not needed.
10. The protocol becomes extensible to any other event kind to be transmitted privatedly by itself (private likes, private reports, private long-form-content, etc)
...
This protocol uses two main concepts to protect the transmission of a target event: Sealed Gossip Events and Gift Wraps ([NIP-59](https://github.com/nostr-protocol/nips/pull/468)). The target event is first **unsigned** (to make it an unverifiable if it leaks -> "Gossip"), then Sealed and then GiftWrapped.

As an early example: this is the flow for a private message: 
...
4. Gift Wrap		-> To Receiver: <ENCRYPTED Sealed Gossip>
...
A Sealed Gossip is a `kind:13` event that wraps an unsigned event (the Gossip) with the sender's regular key. The Sealed Gossip is **always** encrypted to a receiver's pubkey but there are no `p-Tag` associated to this event. There is no way to know who the Gossip is for without the receiver or the sender's private key. The only public information in this event is who is signing it. 
...
The enryption algorithm MUST use a versioned payload with a XChaCha chiper. 

## 3. Gift Wrap Event Kind

A GiftWrap event is a `kind:1059` event which wraps any another event with a single use (random) key and is `p`-Tag-addressed to the receiver.
...
GiftWraps alone are an effective tool to hide information from the public. However, the inner event can be easily re-broadcasted, breaking the privacy of sender and leaking the entire metadata about the inner event (e.g. author, event kind, dates, tags, etc). To make sure the receiver cannot re-broadcast, the GiftWrap MUST wrap a Sealed Gossip Event. 
...
To wrap an event for Sealed Gossip or GiftWrap, first serialize it to JSON. Then aquire a shared secred between the signing private key and the receipient's public key, a 24-byte nonce and encrypt it with XChaCha (XChaCha is different than ChaCha). Convert both the nonce and the resulting byte array to Base64 and output an object with the following fields: 
...
To send a private message, create a kind `Kind:14` with the message in it's `.content` and a p-Tag to the receiver. Other tags are encouraged to be added, such as `content-sensitivity` if the content is NSFW and `subject` to create a name to the conversation. 
...
1. Create a new key, serialize the seal and encrypt into a `kind:1059` with a tag to the receiver, signed by the new random key. 
2. Broadcast the `kind:1059` event to the relays of the receiver. 

For the sender: 

1. Create a new key, serialize the seal and encrypt into a `kind:1059` with a tag to the sender, signed by the new random key. 
2. Broadcast the `kind:1059` event to the relays of the sender. 

## 6. Private Group Chats

Private Group chats are `Kind:14`s for more than one `p` tag. There is no channel definition event. The group of authors + p' tags are a group. 

To send a private message to a group, create a kind `Kind:14` event with the message in it's `.content` and add a p-Tag to each receiver. Other tags are encouraged to be added, such as `content-sensitivity` if the content is NSFW and `subject` to create a name to the conversation. Once `Kind:14` is created, remove the signature and serialize as JSON.
...
2. Create a new key, serialize the seal and encrypt into a `kind:1059` with a tag to the **receiver**, signed by the new random key. 
...
Clients SHOULD render every different group of (author + p' tags) as a group with continuing conversations. If a new p tag is added, a new group is created. 
...
## 5. P2P Message Example, Step By Step
...
```json
   {
      ...
      "tags":[
         [
            "p",
            "7185de5d1f6ca0787b689738bc6895e88f88781e4e1961400742fbfbc19c6863"
         ]
      ]
   }
```
...
```json
   {
      ...
      "tags":[
         [
            "p",
            "0956f4d1f65968e303f22220382550ef5979de1a4f0f136023c947d21a344bf6"
         ]
      ]
   }
```

10. Broadcast the 2 `kind:1059` GiftWraps, delete all the other events. 

## 6. Private Group Chat, Step by Step
...
```json
   {
      ...
      "tags":[
         [
            "p",
            "063491fa5da40f7766802ef33311428332754372999a6f8411df2420fc7a5f01"
         ],
         [
            "p",
            "b3f12cf20281a8c74054adad8a3f4f47618ef4ebac6d9b1767ea7296b7802770"
         ]
      ]
   }
```
...
```json
   {
      ...
      "tags":[
         [
            "p",
            "2d5ad8fb75c30aeb153e6d8b6a7275424be54d196f4837d658cdd36c81dad8d3"
         ]
      ]
   }
```
