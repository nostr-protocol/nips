...
These are extra fields not specified in NIP-01 that may be present in the stringified JSON of metadata events:
  ...
  - `display_name`: a preferred name for the user creating the event. Implementations should fall back to `name` (defined in [NIP-01](NIP-01.md)) when this is not available.
  - `website`: an URL related in any way to the event author.
