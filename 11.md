...
```json
{
  ...
  "nip9a": <NIP-9A rules pointer, e.g. "34550:<hex64-pubkey>:<d-identifier>">
}
```
...
A relay that enforces a [NIP-9A](9A.md) `kind:34551` rules document
against inbound writes MAY publish a `nip9a` pointer in this document.
...
```jsonc
{
  "nip9a": "34550:<hex64-pubkey>:<d-identifier>"
  // other fields...
}
```
...
implement NIP-9A SHOULD, on relay-info fetch, resolve the pointer to
...
NIP-9A MUST ignore this field (per the "MUST ignore unknown fields"
rule above).
