...
- `default_query_limit`: The maximum returned events if you send a filter with the limit set to 0.

- `max_query_limit`: if you send send a filter with a `limit` more than max query limit, limit will be replaced by max query limit.
...
```json
{
  ...
  "posting_policy": "https://jellyfish.land/tos",
