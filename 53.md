...
Service providers SHOULD keep the participant list limited (preferably less than 1000 users). Upon reaching capacity, providers SHOULD determine which participants are specifically mentioned in the event. Clients should not anticipate a full participant list. After the event concludes, it can be deleted or updated to summarize the event and offer asynchronous content (for example, a recording of the event). 
