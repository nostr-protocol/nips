...
### Blob Storage Community Fallback

Kind `4242` is a normal event used to announce that the author has saved one or more blobs from another event on their own Blossom servers. It SHOULD be a reply to the event containing the blobs, using an `e` tag.

The event MUST contain one or more `x` tags with the SHA-256 hashes of the saved blobs, and one or more `server` tags with the URLs of the Blossom servers where the blobs can be found.

```yaml
{
  "kind": 4242,
  "tags": [
    ["e", "<event-id>", "<optional-relay-url>"],
    ["x", "<sha256-hash-1>"],
    ["x", "<sha256-hash-2>"],
...
Clients MAY use these events as additional hints when retrieving the tagged blobs. They SHOULD verify each downloaded blob's SHA-256 hash before using it.
