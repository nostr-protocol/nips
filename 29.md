...
A group's policy is described by three sections, **admission**, **visibility** and **participation**, plus two escalating emergency overrides, **lock** and **quarantine**. Each state is a single one-word tag on the relay-generated `kind:39000` metadata event.
...
- A *missing* section is read as its most open value, the first one in the table; a group SHOULD nonetheless state every value explicitly.
