...
```jsonc
// Admin attaches "nostr" under "tech"
{ "kind": 9002, "tags": [["h", "nostr"], ["parent", "tech"]] }
// Relay emits:
{ "kind": 39000, "tags": [["d", "nostr"], ["name", "Nostr"], ["parent", "tech"]] }

// Admin moves "nostr" under "social" (re-parent)
...
Each subgroup has its own independent membership: users join via `kind:9021` → `kind:9000` like any other group. Relays MAY implement member cascading (for example, emitting `kind:9000` on a subgroup when a member is added to the parent) through normal moderation events.
