...
- *room creation permission* (`kind:19004`) (optional)
...
A `p` tag must be included for every user with permission to create a new room. If this event does not exist, it should be assumed that anyone can create a room. Relays MAY generate this on demand and include only the `p` tags relevant to the authenticated user or users p-tagged in the filter.
