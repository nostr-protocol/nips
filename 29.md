...
- `public` signals the group can be _read_ by anyone, while `private` signals that only members can read. If omitted, groups should be considered `public` by default.
- `open` signals that the group can be _written to_ by anyone, while `closed` signals that only members can write. If omitted, groups should be considered `open` by default.
