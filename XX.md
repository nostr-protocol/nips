...
This NIP defines a Quorum as an nsec shared among a public or private group of members. This is accomplished through a threshold multi-signature scheme (T of N) where the members can chose to represent themselves with their own share of the secret, or they can delegate this power to a council of representatives.
...
```
{
  ...
  "pubkey": "<32-bytes lowercase hex-encoded public key of the >",
  ...
  "tags": [
	...
	// chain of governance
	["p", "<32-bytes hex of previous_governanance_npub>", "<optional recommended relay URL>", "governance_npub1"],
	["p", "<32-bytes hex of current_governanance_npub>", "<optional recommended relay URL>", "governance_npub2"],
...
When the council of representatives changes, the old nsec and its secret shares are made obsolete and a new nsec is shared to the new council. The final act of any council is either signing the update `event:xxx` which passes on the chain-of-governance to the next council. If the council fails to do this, the decision of how to proceed is pushed to the members themselves.

A client only needs the most recent `event:xxx` to function, but will need the entire history of `event:xxx` updates to verify the chain-of-governance.
...
If someone wishes to become a member of a public quorum, they submit `event:yyy`. For a private quorum, this request must be approved by the quorum in a similar fashion as the moderated community event `event:4550`.

Similarly, any member may submit a resignation event.


### Quorum "Ontologies"

A quorum may be a member of another quorum, which enables the construction of multi-quorum structures. For example, a each locality may have a quorum, and those quorums can join together by becoming members of a regional quorum.


### Elections / Polls

The governance is enforced by the threshold multi-signature, however, polls and elections may used to in a variety of ways:
...
An optional tag in the Quorum `event:xxx` is used to define a list of laws. At the time of this writing, no formal system of the laws is proposed but it entirely possible. As a starting point, laws can be expressed in natural language to codify shared intentions. For example, the laws could include a Declaration of Independence, Bill of Rights and Constitution.

Example of an already formalized law:
  - "An event may be signed by the quorum's npub only if a quorum of the council has been achieved ."
  - "All members of this quorum must be approved by the council."

Some examples of natural language laws:
...
Example use cases:
 ...
 - Governance (Judicial Systems, Treasuries, Trusts, Countries, etc)
 - More resilient & dynamic moderated community
...
NIP-29: Shared Event Ownership through DVMs
