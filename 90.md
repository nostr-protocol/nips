...
Job results always use a kind number that is `1000` higher than the job request kind. (e.g. request: `kind:5001` gets a
result: `kind:6001`).
...
Nostr can act as a marketplace for data processing, where users request jobs to be processed in certain ways (e.g., "
...
* `i` tag: Input data for the job (zero or more inputs)
    ...
    * `<input-type>`: The way this argument should be interpreted. MUST be one of:
        ...
        * `job`: The output of a previous job with the specified event ID. The dermination of which output to build upon
    ...
    * `<relay>`: If `event` or `job` input-type, the relay where the event/job was published, otherwise optional or
      empty string
...
* `param`: Optional parameters for the job as key (first argument)/value (second argument). Different job request `kind`
...
If the user wants to keep the input parameters a secret, they can encrypt the `i` and `param` tags with the service
...
* `amount`: millisats that the Service Provider is requesting to be paid. An optional third value can be a bolt11
  invoice.
* `i`: The original input(s) specified in the request.

## Encrypted Output

If the request has encrypted params, then output should be encrypted and placed in  `content` field. If the output is
...
* `status` tag: Service Providers SHOULD indicate what this feedback status refers
  to. [Job Feedback Status](#job-feedback-status) defines status. Extra human-readable information can be added as an
  extra argument.

* NOTE: If the input params requires input to be encrypted, then `content` field will have encrypted payload with `p`tag
  as key.
...
Any job feedback event MIGHT include results in the `.content` field, as described in
...
Job feedback (`kind:7000`) and Job Results (`kind:6000-6999`) events MAY include an `amount` tag, this can be
...
Customers can always either pay the included `bolt11` invoice or zap the event requesting the payment and service
providers should monitor for both if they choose to include a bolt11 invoice.

## Notes about the protocol flow

The flow is deliberately ambiguous, allowing vast flexibility for the interaction between customers and service
...
A Customer MAY request multiple jobs to be processed as a chain, where the output of a job is the input of another
