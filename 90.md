...
*   **Response:** The DVM responds with a `kind: 26xxx` (corresponding to the request kind) event. The `content` contains a JSON-RPC response. The `result` field MUST follow the Standard Output Object Types, containing an array with a single object: `[{ "type": "json", "data": "<JSON_Schema_String>" }]`. In case of failure, no job result event should be sent, and instead a job feedback event with a `["status", "error"]` tag MUST be used.
...
*   **Content:** A JSON-RPC 2.0 response string.
    *   `result`: Contains the job output. MUST be an array of objects, each having a `type` field and corresponding data field(s).
*   **Tags:**
...
**Example Job Feedback Event (Kind 27000 - Error):**
...
7.  Handle error responses in `kind: 27000` feedback events with `["status", "error"]` tag.
8.  If payment is requested (`kind: 27000` with `["status", "payment-required"]` tag), handle the payment flow (e.g., NIP-57 zap receipt).
...
2.  Format `content` of Job Results as a valid JSON-RPC 2.0 response string (with `result`). For errors, use Job Feedback events with `["status", "error"]` tag and include error details in the `content`.
...
5.  Validate incoming `params` against its schema. Reject invalid requests with a Job Feedback event (`kind: 27000`) with `["status", "error"]` tag and include a JSON-RPC error object in the `content`.
6.  Respond to `get_schema` requests with the schema string wrapped in the standard output format (`[{ "type": "json", "data": "<Schema_String>" }]`) in the `result` field of a `kind: 26xxx` event. If the request cannot be fulfilled, respond with a Job Feedback event (`kind: 27000`) with `["status", "error"]` tag.
...
3.  Provide job status updates via `kind: 27000` Feedback events, especially for long jobs. MUST use Feedback events with `["status", "error"]` tag for all error conditions instead of including errors in Job Result events.
4.  Use `kind: 27000` with `["status", "payment-required"]` tag and payment tags (`amount`, `bolt11`, `cashuReq`) to request payment. Wait for payment confirmation (e.g., NIP-57 zap) before proceeding.
