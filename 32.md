...
When a more practical classification is a subclass relationship, `P279` (`subclass of`) can be used in the same way as
`P31`. In principle any Wikidata property can serve as the predicate in the `L` tag when a vocabulary expects a
well-defined statement structure.
