...
- `created_at` [Created At]: The unix timestamp when the order was created. Unlike the event's `created_at`, which changes on every update of this addressable event, it MUST stay the same across updates, so clients can show the order's age and sort by it.
