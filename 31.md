...
Next, he takes the event JSON, encrypts it using `skBob` and `pkAlice` and wraps it in a `kind: 1004` incognito DM using his `pkBob''` pubkey.
...
On Alice's side, her client can subscribe to `kind: 1004` events from `pkBob''` on Bob's trusted relay. Upon receipt of an event, the client can decrypt the `.content` and unwrap the regular DM from Bob and present it to Alice. For added privacy, the individuals could choose to use private relays.
...
In theory, the user's nostr private key could be used as the seed, but since `window.nostr` and physical signing devices prevent direct access to private keys, it's not desirable.
...
Given an integer `counter` which starts at `0`, we increment the counter by 1 for each new pubkey, the two disposable identities will be derived from a seed `seed` (256 bits of entropy) using the following derivation paths:

`pkBob' ` = `m/44'/0'/<counter>'/0/0`
`pkBob''` = `m/44'/0'/<counter>'/0/1`

The `counter` increments the account, and the final number is the index which is used to generate new addresses.
