...
The theoretical maximum plaintext size is 2^32 - 1 bytes (~4 GB). Implementations SHOULD enforce
their own maximum payload size based on platform and resource constraints, rejecting oversized payloads
early in `decode_payload` (before base64 decoding) to prevent denial-of-service. Decryption may require
several times the payload size in working memory due to base64 decoding, byte array slicing, and
padding operations. For reference, JVM-based systems are limited to ~2 GB contiguous arrays, and mobile
devices may have significantly less available memory. Note that `calc_padded_len` can return values up
to 2^32, which exceeds the range of unsigned 32-bit integers; implementations must use 64-bit (or
larger) arithmetic for padding calculations.
...
| plaintext_len | prefix | padded_len | plaintext_sha256 | payload_sha256 |
|---|---|---|---|---|
| 65535 | u16 (2 bytes) | 65536 | `6e1bebca6a8229364a162a72ef064826c4cd7457bf54f190ef782bd9deff3e42` | `6d8c2810d1e870fbaa1f0a0937126cca837a15f9260e27060c331d70a3c0bc84` |
| 65536 | extended (6 bytes) | 65536 | `bf718b6f653bebc184e1479f1935b8da974d701b893afcf49e701f3e2f9f9c5a` | `b7b4edb36ba92e267d322d56d9aebc22e7fa96ff52e3c12adc07f07a43cbc616` |
...
Note that 65535 and 65536 both have a `padded_len` of 65536, but the total padded-with-prefix
sizes differ: 65538 (2 + 65536) vs 65542 (6 + 65536). The jump to 65537 triggers the next
padding bucket at 81920.
