...
* `ox` containing the SHA-256 hexencoded string of the original file, before any transformations done by the upload server; it should include the root URL of each of the NIP-96 compatible HTTP file storage servers used on upload.
...
```json
{
  ...
  "tags": [
    ...
    ["m", <MIME type>],
    ["x",<Hash SHA-256>],
    ["ox",<Hash SHA-256>, <HTTP server URL 1>, <HTTP server URL 2>],
