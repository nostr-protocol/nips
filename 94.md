...
Kind 1063's may be published directly to a relay to create a standalone image reference, or JSON-encoded and attached to another event using the `imeta` tag. This serves to add metadata to an image embedded in a text note's `content`, or to describe file "attachments" not directly referenced by the note's `content`.
