...
Both have the same structure and tags. The unique features of the private version will be explained on the next section.
...
**Table of "Set Item Reference" Event Kinds and Structure by Referenced Item Type**: Note that `obfuscate(<value>)`
is representing a step taken to hash the `d` tag value when building private "Set Item Reference" events,
...
| **event** (private) | 31383 | obfuscate(`<event-pubkey>:<event-id>`) | `["k", "<event-kind>"]` |
